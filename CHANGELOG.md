...
- Multi-proveedor: OpenAI, Anthropic/Claude, DeepInfra, OpenRouter, Groq, Ollama y cualquier API OpenAI-compatible. `/connect <id> [baseUrl]`, `/provider [id]`. Health `{ llm, provider }` (alias `deepseek`).
- CLI: en chat, `/model` (o click en MODEL del header) abre una lista para elegir el modelo que responde (`deepseek-v4-flash` / `pro` / `vision`).
- Tokens y contexto: el back acumula `usage` de cada llamada LLM (API o estimado chars/4). El TUI muestra CTX/IN/OUT/JOB/SES en SYSTEM; `/status` y el cierre del job también. `/new` resetea el acumulado de la sesión.
- TUI P0–P2: header compacto (sin THREADS), empty state con acciones, footer contextual, fase del job en el prompt. SYSTEM compact/full/off (`ctrl+p`), colores por rol, paleta `ctrl+k`, traces colapsables, `/copy` (`ctrl+y`), overlay first-run, `NO_COLOR` / `SQUAD_CONTRAST=high`.

### Fixed

- CLI: en el formulario de rules, Backspace borra a la izquierda (Linux manda 0x7f como `key.delete`; antes solo andaba Ctrl+Backspace).

### Changed

- CLI: el EXPLORER lista carpetas primero y archivos después (como VS Code), con badge de tipo (TS, JA, {}, ▣ carpeta). Sin Nerd Font: el TTY no las tiene.
- CLI: fondo del TUI negro puro (`#000000`). El alt-screen fija el bg default del TTY para que las celdas vacías no titilen.
- CLI: reloj, CPU/RAM y spinner ya no re-pintan el TUI entero; Ink solo reescribe las líneas que cambiaron.
...
- CLI: `squad` arranca SquadAi-Back si localhost no responde, y lo apaga al salir.
